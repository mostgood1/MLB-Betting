        ...
        for path in possible_paths:
            if os.path.exists(path):
                try:
                    ...
                    print(f"[OK] Team assets loaded from {path}")
                    break
                except Exception as e:
                    print(f"[WARNING] Error loading team assets from {path}: {str(e)}")
        else:
            print("[WARNING] No team assets file found!")
...
def get_team_assets(team_name: str) -> Dict[str, Any]:
    ...
    # Normalize the team name to ensure consistent lookups
    normalized_team_name = normalize_team_name(team_name)
    
    # Use the team assets manager to get the team's assets
    team_assets = _team_assets_manager.get_team_assets(normalized_team_name)
    ...
    print(f"⚠ Team assets not found for: {team_name} (normalized: {normalized_team_name})")
